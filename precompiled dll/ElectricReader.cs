...
namespace ElectricReader
{
    public class ElectricReader : Mod
    {
        ...
        public override string Name => "ElectricReader"; //You mod name
        public override string Author => "Jayy"; //Your Username
        public override string Version => "1.0"; //Version
        public override string Description => "More convenient and performance friendly Bill viewer mod."; //Short description of your mod
        ...
        {
            Settings.AddHeader((Mod)this, "UI Elements");
            this.my_checkbox = Settings.AddCheckBox((Mod)this, "my_checkbox", "Enable UI", this._active);
        }
            ...
            else
            {
                foreach (RaycastHit raycastHit in Physics.RaycastAll(this.cam.transform.position, this.cam.transform.forward, 3f))
                {
                    if ((UnityEngine.Object)raycastHit.collider.gameObject == (UnityEngine.Object)this.readere)
                    {
                        ...
                        PlayMakerGlobals.Instance.Variables.GetFsmString("GUIinteraction").Value = string.Format("Electric Bill: {1}", (object)this._dunkwh, (object)this._donedecimalless);
                    }
                }
            }
        }
        ...
        private void Mod_UI()
        {
            ...
            {
                GUI.Label(new Rect(5f, 125f, 2000f, 80f), string.Format("Electric Bill: {0}mk", (object)this._donedecimalless));
            }
            else
                GUI.enabled = false;
            if (this._firehouse)
                GUI.Label(new Rect(5f, 145f, 2000f, 80f), "SHIT YOUR HOUSE IS BURNING!!!!");
